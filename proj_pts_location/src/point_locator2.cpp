...
public:
  PointLocator2(const rclcpp::NodeOptions &options)
    ...
    bbox_sub_ = this->create_subscription<vision_msgs::msg::BoundingBox2D>(
        "/apple_detector/bbox_coords", 10,
        std::bind(&PointLocator2::bboxCallback, this, _1));
...
private:
  ...
  std::mutex mutex_;
  cv::Mat depth_image_;
  vision_msgs::msg::BoundingBox2D latest_bbox_;
  ...
  void loadIntrinsicsFromYAML() {
    std::string package_share =
        ament_index_cpp::get_package_share_directory("proj_pts_location");
    std::string config_path = package_share + "/config/camera_intrinsics.yaml";

    try {
      YAML::Node config = YAML::LoadFile(config_path);
      auto K_color =
          config["camera_intrinsics"]["K_color"].as<std::vector<double>>();
      auto K_depth =
          config["camera_intrinsics"]["K_depth"].as<std::vector<double>>();
      ...
      RCLCPP_ERROR(this->get_logger(), "Failed to load intrinsics: %s",
                   e.what());
    }
  }

  // Receive bounding boxes
  void bboxCallback(const vision_msgs::msg::BoundingBox2D::SharedPtr msg) {
    std::lock_guard<std::mutex> lock(mutex_);
    latest_bbox_ = *msg;
    got_bbox_ = true;
    wait_for_depth_ = true;
    RCLCPP_INFO(get_logger(), "BBox received: x=%lf y=%lf w=%lf h=%lf",
                latest_bbox_.center.position.x, latest_bbox_.center.position.y,
                latest_bbox_.size_x, latest_bbox_.size_y);
  }
  ...
  void compute3DPoint(const rclcpp::Time &depth_stamp) {
    if (!(got_depth_info_ && got_color_info_ && got_depth_ && got_bbox_))
      return;
    ...
    auto u_center = latest_bbox_.center.position.x;
    auto v_center = latest_bbox_.center.position.y;

    auto width_offset = latest_bbox_.size_x / 2;
    auto height_offset = latest_bbox_.size_y / 2;
    ...
    if (count > 0) {
      ...
      if (tf_buffer_.canTransform("map", point_msg_color_cam.header.frame_id,
      ...
      RCLCPP_INFO(
          get_logger(),
          "Avg 3D point in map frame: [%.3f, %.3f, %.3f] from %d points",
          point_msg_map.point.x, point_msg_map.point.y, point_msg_map.point.z,
          count);
    } else {
