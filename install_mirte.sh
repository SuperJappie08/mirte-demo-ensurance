...
if [[ -n DEBUG_simulate ]]; then
	echo "$_debug ADDING APT REPOS"
	echo "$_debug deb [trusted=yes] https://github.com/ArendJan/mirte-ros-packages/raw/ros_mirte_humble_jammy_${_arch}_extra_packages/ ./"
	echo "$_debug yaml https://github.com/ArendJan/mirte-ros-packages/raw/ros_mirte_humble_jammy_${_arch}_extra_packages/local.yaml humble"
else
...
echo "$_done Succesfully added apt repot for ros2-system-monitor"

echo "$_info Updating and Installing new packages"
if [[ -n DEBUG_simulate ]]; then
	echo "$_debug UPDATING PACKAGES AND INSTALL"
else
	sudo apt update
	sudo apt upgrade -y
...
if [[ ! -e /home/mirte/mirte_ws/src/mirte-ros-packages/ ]]; then
	...
	[[ -n DEBUG_simulate ]] && echo "$_debug extra_path=$extra_path"
